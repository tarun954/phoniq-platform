  ...
  async function submit(event){
    ...
    try{
      ...
      if(!check.ok){
        ...
        throw new Error(result?.error||"This account is not a Phoniq internal user.");
      }
    ...
    <main style={page}>
      <section style={card}>
        <div style={{display:"flex",gap:12,alignItems:"center"}}>
          ...
          <div><strong style={{fontSize:18}}>PHONIQ</strong><small style={small}>ADMIN CONSOLE</small></div>
        </div>
        <p style={eyebrow}>INTERNAL SIGN IN</p>
        <h1 style={{fontSize:34,margin:"6px 0"}}>Phoniq Admin</h1>
        <p style={{color:"#64748b"}}>For Phoniq employees and platform administrators only.</p>
        ...
        <form onSubmit={submit}>
          ...
          <button style={button} disabled={working}>{working?"Signing in...":"Sign In to Admin Console"}</button>
        </form>
        <p style={{marginTop:18,color:"#64748b",fontSize:13}}>
          Client user? <Link href="/login">Use Client CRM login</Link>
        </p>
      </section>
    </main>
  );
...
const page={minHeight:"100vh",display:"grid",placeItems:"center",background:"#0f172a",padding:24,fontFamily:"var(--font-geist-sans),Arial,sans-serif"};
const card={width:"min(460px,100%)",background:"#fff",borderRadius:20,padding:30,boxShadow:"0 24px 70px rgba(0,0,0,.3)"};
